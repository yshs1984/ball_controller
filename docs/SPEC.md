...
- 側面はY方向の中間(`y=0`)で上下2段に分け、`MeshRenderer`に2つの材質(`sharedMaterial`=既存の米色`BallMaterial`、`noriMaterial`=Inspectorでアサインする黒い`NoriMaterial`)を割り当て、メッシュを2つのサブメッシュに分けて描画している。下段(下フタ含む)が海苔、上段(上フタ含む)が米
